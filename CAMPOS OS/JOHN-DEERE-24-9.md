...
- `/ciclo/john-deere/actividad?clave=cd-monedas`: la 3ª y la 2ª más votadas (placa 26)
- `/ciclo/john-deere/actividad?clave=cd-monedas&vista=primera`: la 1ª sola, con la card de quien la escribió (placa 27)
- `/ciclo/john-deere/actividad?vista=cierre`: los números del role play

**El deck**: `public/pres/johndeere-conversaciones.html`, **30 placas** en el
orden del reloj. `data/presentaciones.json` declara 30. Se abre desde el hub, que
...
la 4, traducido en la 8, conversación en la 9, traducción en la 17, las tres
rondas del ensayo en 20, 21 y 22, la pregunta en la 24, las monedas en la 25, el
reparto en la 28 y la encuesta final en la 30. El 23/9 se sacaron la apuesta
("¿es un hecho?"), con su placa, y las placas "Conducir un equipo puede ser un
espacio solitario" y "Qué hace que una devolución funcione". Hasta qué placa queda abierta cada actividad va en
...
cargado en la tabla `guiones` con las 30 placas: cada cambio al archivo se vuelve
a cargar ahí.

**La clave del panel de control** está en la fila de la corrida, en la tabla
`corridas`. No se versiona: el repositorio es público.

### El bloque de las preguntas (placas 24 a 28)

- **24**: cada uno escribe su pregunta en el teléfono.
- **25**: se abren los Deer Coins y la sala vota mientras pasan las respuestas de
  los que llevan años.
- **26**, teléfonos cerrados: llega con la 3ª más votada; el botón "Mostrar la 2ª"
  suma la 2ª. Una persona contesta cada una.
- **27**: la 1ª sola, en dos cards. **Llegar a esta placa es lo que la revela** y
...
- **28**: el reparto de las que quedaron.

El paso del ranking vive en `corridas.revelado` (0 la 3ª, 1 hasta la 2ª, 2 la
1ª) y **solo avanza**. Abrir la pantalla de la placa 27 en cualquier navegador
lo pone en 2.
